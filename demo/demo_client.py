...
class StartTest(QThread):
    ...
    def run(self):
        while self.running:
            if self.start_signal_list[self.work_pos_index][0] == 1 and self.start_signal_list[self.work_pos_index][1] == 0:
                        ...
                        if self.running:
                            if self.para_list[i] not in self.else_test[1:]:
                                if self.data_list[self.current_step][i][0] == 1:  # 配方信息等于1表示该属性得电
                                    ...
                                    self.keep_time = self.data_list[self.current_step][i][1]  # 记录延迟时间
                                    # get相应参数当前值
                                    self.slot = '{},{},{:.2f},{:.2f},{:.2f}, {:0>2.0f}'.format(self.codesys_work_pos_list[self.work_pos_index][i],
                                         self.count, time.perf_counter(), start, self.keep_time, time.perf_counter() - start_time)
                                    # 判断相应参数是否一致
                                ...
                                if self.data_list[self.current_step][i][0] != 0:  # 开始检测
                                    ...
                                    self.slot = '{},{},{:.2f},{:.2f},{:.2f}, {:0>2.0f}'.format(self.codesys_work_pos_list[self.work_pos_index][i],
                                         self.count, time.perf_counter(), start, self.keep_time, time.perf_counter() - start_time)
                                if self.codesys_work_pos_list[self.work_pos_index][i] < self.data_list[self.current_step][i][0]:
